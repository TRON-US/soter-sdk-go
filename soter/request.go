...
func (r *Request) Send(c *http.Client) (*Response, error) {
	...
	if resp.StatusCode >= http.StatusBadRequest {
		...
		case contentType == "text/plain":
			...
			if err != nil {
				fmt.Fprintf(os.Stderr, "soter-shell: warning! response (%d) read error: %s\n", resp.StatusCode, err)
			}
			e.Message = string(out)
		case contentType == "application/json":
			if err = json.NewDecoder(resp.Body).Decode(e); err != nil {
				fmt.Fprintf(os.Stderr, "soter-shell: warning! response (%d) unmarshall error: %s\n", resp.StatusCode, err)
			}
		default:
			fmt.Fprintf(os.Stderr, "soter-shell: warning! unhandled response (%d) encoding: %s", resp.StatusCode, contentType)
			out, err := ioutil.ReadAll(resp.Body)
			if err != nil {
				fmt.Fprintf(os.Stderr, "soter-shell: response (%d) read error: %s\n", resp.StatusCode, err)
			}
			e.Message = fmt.Sprintf("unknown soter-shell error encoding: %q - %q", contentType, out)
		}
		nresp.Error = e
		nresp.Output = nil
